...
describe('Genre e2e test', () => {
    ...
    it('should load Genres', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="genre"]')).first().click().then(() => {
            const expectVal = /Genres/;
    ...
    it('should load create Genre dialog', () => {
        element(by.css('button.create-genre')).click().then(() => {
            const expectVal = /Create or edit a Genre/;
